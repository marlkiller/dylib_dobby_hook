...
- (void)firstLaunch{
    NSString *activationCode = @"TP-12345678-1234-1234-1234-123456789012";
    NSAlert *alert = [[NSAlert alloc] init];
    alert.messageText = @"Offline License Key";
    alert.informativeText = [NSString stringWithFormat:@"Your license key is:\n%@", activationCode];
    [alert addButtonWithTitle:@"Copy License Key"];
    [alert addButtonWithTitle:@"Close"];
    NSModalResponse response = [alert runModal];
    if (response == NSAlertFirstButtonReturn) {
        NSPasteboard *pasteboard = [NSPasteboard generalPasteboard];
        [pasteboard clearContents];
        [pasteboard setString:activationCode forType:NSPasteboardTypeString];
    }
};
...
- (id)hk_dataTaskWithHTTPMethod:(NSString *)method
    ...
    if ([URLString containsString:@"tableplus.com"]) {
        ...
        for (NSURLQueryItem *item in components.queryItems) {
            if ([item.name isEqualToString:@"deviceID"]) {
                deviceID = item.value;
                break;
            }
        }
        if (!deviceID && [parameters isKindOfClass:[NSDictionary class]]) {
            id paramDevice = parameters[@"deviceID"];
            if ([paramDevice isKindOfClass:[NSString class]]) {
                deviceID = paramDevice;
            }
        }
        if (!deviceID) {
            deviceID = @"DEFAULT_DEVICE_ID_FALLBACK";  // [UIDevice currentDevice]
        }
        
        if ([URLString containsString:@"v1/licenses/devices"]) {
            NSMutableDictionary *result = [NSMutableDictionary dictionary];
            [result setObject:deviceID forKey:@"DeviceID"];                  // 动态 deviceID
            ...
            success(nil, @{
                @"Data": @{
                    ...
                    @"NeedToUpdate": @NO,
                    @"PushLocalNoti": @NO,
                    @"Tittle": [Constant G_EMAIL_ADDRESS],
                    @"LicenseKey": @"TP-12345678-1234-1234-1234-123456789012"
                }
            });
        ...
        } else if ([URLString containsString:@"osx/version.xml"]) {
            success(nil, @{
                @"Data": @{
                    @"version": @"1.0.0",
                    @"build": @"100",
                    @"releaseDate": @"2026-12-30",
                    @"downloadURL": @"https://tableplus.com/download"
                },
                @"Code": @200
            });
        } else {
